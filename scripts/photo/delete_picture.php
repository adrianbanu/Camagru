...
if (isset($_GET['id-photo']) && $_GET['id-photo'] != NULL && is_numeric($_GET['id-photo']))
{
	try{
		...
		{
			$requete = $bdd->prepare("SELECT `id_photo` FROM `photos` WHERE `id_photo` LIKE :id_photo");
...
if ($_SESSION['groupe'] == 'admin')
{
	...
	echo "<meta http-equiv='refresh' content='0,url=../works/works-users.php?login=".$_SESSION['login']."'>";
}
